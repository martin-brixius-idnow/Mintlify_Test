...
This solution template enables users with previously verified identities (e.g. acquired in bank onboarding process) to sign documents without repeating full identity verification. It uses device trust signals to determine whether the user’s device and network are trusted enough to allow an electronic signature without requiring an immediate identity verification..
