...
In TrustPlatform, each flow runs as a session made up of one or more nodes, like identity checks or digital signatures. 

Instead of polling the API, your server gets an HTTP POST as soon as something changes. Because nodes run asynchronously, depending on the user, the device, or third-party services, webhooks keep your system in sync and let you know when new results or data blocks are ready to retrieve.
...
**Retry conditions:**
- âœ… Transient errors (5xx status codes)
- âœ… Network errors (timeout, connection refused)
- âœ… Endpoint unavailable

**No retry for:**
- âŒ Client errors (4xx status codes)
- âŒ Authentication failures
- âŒ Invalid endpoint URL
...
<CardGroup cols={2}>
  <Card title="Secure Webhooks" icon="lock" href="/webhooks/secure">
