---
title: Capture with Instant signature issuance
...
The users can complete document-based IDV through agent-assisted review (VideoIdent), automated visual verification (AutoIdent), combined with either contract signing (eSign) or qualified electronic signature (QES). Each mode ensures compliance with regulations such as eIDAS, AML, GwG, and BaFin. 
...
- ***VideoIdent + Contract Signing*** - Agent-assisted document verification combined with electronic contract signing (VideoIdent + eSign).
- ***AutoIdent + Contract Signing*** - Automated document verification combined with electronic contract signing (AutoIdent + eSign).
- ***AutoIdent + QES*** - Automated document verification with Qualified Electronic Signature (trust-based identity verification; no contract PDF input required). 
