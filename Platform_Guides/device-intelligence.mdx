...
- Integrates **_device signals services_** (e.g., Fingerprint) to assess device risk and detect fraud attempts.
- **_Analyses automation_**, environment manipulation, VMs/emulators, and behavioral anomalies.
- **_Configurable risk thresholds_** via `suspiciousUserThreshold`, `notTrustedUserThreshold`, and `confidenceScoreThreshold`.
- **_Output mapping_** based on `suspectScore` and `confidence.score` thresholds.
