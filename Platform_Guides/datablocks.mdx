---
title: Data blocks
description: Unit of information produced and consumed by flow steps
---
...
| reasons[].code      | Defined by the third‑party Web SDK for the "error" event. Full list: [Error Explanations](https://docs.keyless.io/web-sdk/web-sdk-reference/error-handling#errors-explanation). | Yes |
...
| result  | Final qualitative outcome computed by IDnow. Possible values: trusted, suspicious, not_trusted, inconclusive. Results correlate to step level configuration | Yes |

| score   | Numerical trust score (0–100) computed by IDnow. Based on the confidence score of the third‑party provider multiplied by 100. See also See also: [Confidence Score](https://dev.fingerprint.com/docs/identification-accuracy-and-confidence#confidence-score) | Yes |

| signals | Mandatory JSON object containing the complete, raw, unaltered response from the external provider. More info: [API reference](https://dev.fingerprint.com/reference/server-api-get-event) | Yes |
...
| result                          | Final qualitative outcome computed by IDnow. Possible values: trusted, suspicious, not_trusted, inconclusive. | Yes |

| score                           | Numerical trust score (0–100) computed by IDnow. Based on third‑party provider score divided by 10.See also: [Trustfull Digital Score(https://docs.trustfull.com/docs/what-is-digital-score) | Yes |

| reasons[].code                  | Machine‑readable reason code defined by the third‑party provider. Full list: [Reason Codes](https://docs.trustfull.com/docs/reason-codes#risk-signals). | Yes |

| reasons[].details               | Human‑readable explanation for the reason code, defined by the third‑party provider. Full list: [Reason Codes](https://docs.trustfull.com/docs/reason-codes#risk-signals) | Yes |

| signals                         | Mandatory JSON object containing the complete, raw, unaltered response from the Trustfull [Onboarding API](https://docs.trustfull.com/reference/digital-score-api). | Yes |
...
| result            | Possible values: success or failure. | Yes |
