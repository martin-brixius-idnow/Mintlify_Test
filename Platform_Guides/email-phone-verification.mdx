...
- Computes a **_trust score_** from email, phone, IP and name signals.
- Routes flow using **_configurable thresholds_**: suspiciousUserThreshold and trustedUserThreshold.
- **_Optional `inputSources`_** to specify step IDs (basicIdentity, extendedIdentity, deviceSignals); otherwise uses latest data blocks.
- **_Produces `DigitalSignals`_** for trusted, suspicious, and not_trusted; inconclusive when no definitive assessment.
