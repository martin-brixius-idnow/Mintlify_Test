...
Call the Session API to initiate a flow session.
...
const response = await axios.post(`/flows/${flowId}/${environment}/sessions`, form, {
  headers: {
...
```code-tabs
import requests
url = f"/flows/{flow_id}/{environment}/sessions"
...
    .uri(URI.create("/flows/" + flowId + "/" + environment + "/sessions"))
