...
The core building blocks of IDnow are:

- **Flow**: A flow that runs from start to end and defines the operational steps leading to a final outcome.
- **Start and End steps**: The flow ends in an End step that determines whether the outcome is accepted or rejected.
...
![Minimal flow with Start, a single processing step, and an End step.](/img/core-concepts/flows.png)
...
Between the Start step and the End step, you can add any combination of **processing steps** to perform operations, transform data and control the routing of the flow.

![Minimal flow with Start, a single processing step, and an End step.](/img/1-trustplatform-basic-flow.png)
_End steps represent the final outcome of a flow_
...
Between the Start step and the End step, other **processing steps** perform operations and transform data or control flow branching through **verdicts**.
...
![IDV Step with three outcomes: verified, fraud, and failure, each leading to a dedicated End step.](/img/core-concepts/verdicts.png)
...
![IDV Step producing different data blocks for different verdicts (verified, fraud, failure).](/img/core-concepts/datablocks.png)
...
![Flow where an IDV Step creates a BasicIdentity data block that is used as input for AML Screening, which in turn produces a PepAndSanctionsResults Data block.](/img/trustplatform-aml-screening-match-no-match-alt.png)
...
![IDV Step produces BasicIdentity as an outcome which is used as an inpot for AML Screening](/img/trustplatform-flow-overview.png)
...
![More complex flow where verdicts define flow path and output data blocks created](/img/trustplatform-idv-aml-basicidentity.png)
...
By combining steps, verdicts, data blocks, and routes, you can build complex trust flows. IDnow manages end-user sessions, data propagation and audit trails.
