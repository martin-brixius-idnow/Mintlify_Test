...
- **Operational Steps**: Execute logic such as reading inputs, writing data blocks, and producing verdict
...
_End Steps represent the final decisionOutcome of a flow_
...
The data block structure is preserved in the API response after a flow session, making it easy to trace data from the initial input to the final decisionOutcome.
...
Many data blocks also carry the step’s verdict to indicate the decisionOutcome of the check that generated them.
...
![IDV Step produces BasicIdentity as an decisionOutcome which is used as an inpot for AML Screening](/img/trustplatform-flow-overview.png)
